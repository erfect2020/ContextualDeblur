...
class TinyMIMViT(nn.Module):
    ...
    def forward_encoder_tiny(self, x, y, mask_ratio):
        # embed patches
        x = self.patch_embed(x)
    ...
    def forward(self,  pimgs, gtimgs=None, mask_ratio=0.75,  kqv=False):
        ...
        if gtimgs is not None:
            platent, glatent, ids = self.forward_encoder_tiny(pimgs, gtimgs, mask_ratio)
        else:
            platent, glatent, ids = self.forward_encoder(pimgs)
...
def tinymim_vit_tiny_patch8(**kwargs):
    model = TinyMIMViT(
        patch_size=8, embed_dim=192, depth=12, num_heads=6, drop_path=0.1,last_heads=12,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
